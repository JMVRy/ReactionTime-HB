...
// This is the color to click. GetPixelColor always returns 0 for Alpha, so make sure to specify that if using the == operator
MouseOperations.Color clickColor = new MouseOperations.Color(75, 219, 106, 0);

// Constantly loop every frame. We want to be as quick as possible when clicking!
while ( true )
{
    // Get the current mouse position to use for our check. The mouse doesn't block GetPixelColor, so we don't need to worry about that
    MouseOperations.MousePoint mousePos = MouseOperations.GetCursorPosition();

    // If the color is the exact one we want, then quickly click, and wait (so that we don't click multiple times when waiting for the GPU to update our program with the current pixel color, causing us to miss what our result was)
    if ( MouseOperations.GetPixelColor( IntPtr.Zero, mousePos.X, mousePos.Y ) == clickColor )
    {
